...
class conv_extract_layer(Layer):
    def setup(self, ksize, c, n, stride, pad, batch_norm, activation, inp,
              out):
        if inp is None:
            inp = range(c)
        self.activation = activation
    ...
    def recollect(self, w):
        ...
        assert assert1 and assert2, \
            'Dimension not matching in {} recollect'.format(self._signature)
        self.w['kernel'] = k
        self.w['biases'] = b


class conv_select_layer(Layer):
    def setup(self, ksize, c, n, stride, pad, batch_norm, activation, keep_idx,
              real_n):
